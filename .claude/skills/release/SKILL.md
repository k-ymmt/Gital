...
Argument: the new version (e.g. `/release 1.1`). If no version is given, ask the user which version to release — do not guess.
...
1. **Bump version** (skip if the requested version already matches): update every `MARKETING_VERSION` in `Gital.xcodeproj/project.pbxproj` to the new version and increment every `CURRENT_PROJECT_VERSION` by 1.
