...
- `Gital/GitHub/` — `GitHubService` wraps the `gh` CLI for pull request list/detail/merge. `GitHubAvatars` derives github.com avatar URLs (by login, by commit email); avatars only activate when the default remote's host is github.com.
