...
- `Gital/GitHub/` — `GitHubService` wraps the `gh` CLI for pull request list/detail/merge. `GitHubAvatars` derives github.com avatar URLs (by login, by commit email); avatars only activate when the default remote's host is github.com. `PRViewedState`/`PRViewedStore` track per-PR "Viewed" review progress (commit-level and file-level flags that promote/demote each other), persisted as JSON in Application Support keyed by repo path.
